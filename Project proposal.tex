...
\vspace{-1em}
We suggest that further investigation into the variational continual learning (VCL) approach \cite{vcl} is warranted. Currently, good performance is dependent on re-training on small coresets retained from previous datasets \cite{robust}, but improvements to the approach may eliminate the need for these coresets, which would better reflect true continual learning. % Aesthetic reasons aside - why is this useful?
...
\vspace{-0.75em}
However, the true posterior $p(\theta | \mathcal{D}_{1:t})$ is computationally intractable, so we must approximate it. We suggest that improvements to this approximation may be key to performance. % More - we know the full Bayesian inference is optimal. But this isn't working. So which approximation is the one that breaks it?

% I think normally we'd frame this as being peformed using mean-field variational inference. MFVI minimizes the evidence lower bound (ELBO) loss. That ELBO loss is expressed using a KL divergence between the posterior and variational distribution.
...
April	& Use non-continual BNN to compare diagonal and full model families\\ % Have we already missed this?
May		& Continue with comparison; \\ % You may also need more literature review time. Especially important to be familiar with Barber and Bishop.
...
August	& Continue with MCMC implementation\\% I would set aside some time for understanding the results and interpreting/writing up
